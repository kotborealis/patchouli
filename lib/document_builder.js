...
const {executeLiterate} = require('./literate/literate');
...
module.exports = async (type, targets) => {
    ...
    const enableLiterate = config.literate || config.l;
    if(enableLiterate)
        debug("...and apply literate");

    await concat_markdown(
        targets.sort(),
        info.concat_path,
        enableLiterate ? [executeLiterate] : []
    );
